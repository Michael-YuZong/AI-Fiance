...
def _latest_by_series(records: Iterable[ReviewRecord]) -> List[ReviewRecord]:
    ...
    return sorted(
        latest.values(),
        key=lambda row: (row.series_id, row.round if row.round is not None else -1),
    )
...
def build_review_ledger(root: Path) -> Dict[str, Any]:
    ...
    legacy_round_notes = [row for row in latest_records if row.protocol == "legacy_round_note"]
    legacy_unstructured = [row for row in latest_records if row.protocol == "legacy_unstructured"]
    active_loops = [
        row
        for row in latest_records
    ...
    converged_series = [
        ...
        and row.round is not None  # must have explicit round structure to count as converged loop
    ]
    ...
    return {
        ...
        "summary": {
            ...
            "legacy_round_note_series": len(legacy_round_notes),
            "legacy_unstructured_series": len(legacy_unstructured),
        },
        ...
        "legacy_round_notes": [row.to_dict() for row in legacy_round_notes],
        "legacy_unstructured": [row.to_dict() for row in legacy_unstructured],
...
def render_review_ledger_markdown(ledger: Mapping[str, Any]) -> str:
    ...
    legacy_round_notes = ledger.get("legacy_round_notes", [])
    legacy_unstructured = ledger.get("legacy_unstructured", [])
    ...
    lines = [
        ...
        f"- legacy round-note latest series: `{summary.get('legacy_round_note_series', 0)}`",
        f"- legacy unstructured latest series: `{summary.get('legacy_unstructured_series', 0)}`",
        "",
        "## Latest By Series",
        "",
    ...
    if legacy_round_notes:
        lines.extend(["", "## Legacy Round Notes", ""])
        for row in legacy_round_notes:
            lines.append(f"- `{row.get('series_id', '')}`")
    else:
        lines.extend(["", "## Legacy Round Notes", "", "- none"])

    lines.extend(["", "## Legacy Unstructured", ""])
    if not legacy_unstructured:
        lines.append("- none")
    else:
        for row in legacy_unstructured:
            lines.append(f"- `{row.get('series_id', '')}`")
    return "\n".join(lines).strip() + "\n"
