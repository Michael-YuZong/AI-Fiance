...
from .review_ledger import ReviewRecord, collect_review_records
...
_SOLIDIFICATION_KEYS = ("prompt", "hard rule", "guard", "workflow", "test", "fixture", "lesson", "backlog")
...
ExtraAuditor = Callable[[ReviewRecord], List[ReviewAuditFinding]]
...
def _audit_round_structure(record: ReviewRecord, sections: Mapping[str, str]) -> List[ReviewAuditFinding]:
    ...
    if record.round > 1 and record.previous_round is None:
        findings.append(
            ReviewAuditFinding(
                path=record.path,
                series_id=record.series_id,
...
def _audit_solidification(record: ReviewRecord, sections: Mapping[str, str]) -> List[ReviewAuditFinding]:
    ...
    if needs_solidification and not solidification_text.strip():
        ...
        return findings
    if needs_solidification and not _solidification_categories(solidification_text):
        findings.append(
            ReviewAuditFinding(
                path=record.path,
                series_id=record.series_id,
...
def _round_requires_followup(sections: Mapping[str, str]) -> bool:
    return any(has_actionable_content(sections.get(title, "")) for title in ("主要问题", "框架外问题"))
    ...
    if actionable:
        findings.append(
            ReviewAuditFinding(
                ...
                detail="最新记录已标为 PASS，但正文仍有需要回修的实质问题。",
            )
        )
        if latest.round == 1:
            findings.append(
...
def build_review_audit(root: Path, extra_record_auditors: Iterable[ExtraAuditor] | None = None) -> Dict[str, Any]:
    ...
    for record in audited_records:
        ...
        for auditor in list(extra_record_auditors or []):
            findings.extend(auditor(record))
